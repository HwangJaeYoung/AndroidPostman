...
public class RequestPrimitive {
    ...
    public RequestPrimitive(String operation, String URL, ArrayList<ViewForArrayAdapterForHeaderListView.IHeaderAdd> headerListsItem, String bodyItem) {
        Operation = operation;
        To = URL;
        oneM2MBody = bodyItem;
        ...
        for(int i = 0; i < headerListsItem.size(); i++) {
            ...
            if(headerListsItem.get(i).getHeaderName().toUpperCase().equals("CONTENT-TYPE")) {
                Content_Type = headerListsItem.get(i).getHeaderValue();
            }
        }
    }
